...
import { PublicWhatsAppFloatingAction } from "./public-whatsapp-floating-action";

export function SiteFooter() {
  const { messages } = useLocale();
  const currentYear = new Date().getFullYear();
  const whatsappContact = getPublicWhatsAppContact(
    messages.publicWhatsApp.initialMessage,
  );

  return (
    <>
      <PublicWhatsAppFloatingAction />
            ...
            <div className="mt-4 grid gap-3 text-sm text-muted-foreground [overflow-wrap:anywhere]">
              ...
              {whatsappContact ? (
                <a
                  ...
                  target="_blank"
                >
                  {messages.publicWhatsApp.footerLabel}
                </a>
              ) : null}
            </div>
          </div>
