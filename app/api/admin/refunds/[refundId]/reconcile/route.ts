...
import { reconcileAdminLifecycleAdjustmentRefundIfApplicable } from "@/lib/admin/lifecycle-adjustment-refund-workflow";
import {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const lifecycleResult =
      await reconcileAdminLifecycleAdjustmentRefundIfApplicable(input, actor);
    const result =
      lifecycleResult ?? (await reconcileAdminRefund(input, actor));
