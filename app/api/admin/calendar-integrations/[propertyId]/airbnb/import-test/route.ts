...
const requestSchema = z
  .object({
    candidateImportUrl: z
      .string()
      .trim()
      .min(1)
      .max(AIRBNB_ICAL_URL_MAX_LENGTH)
      .optional(),
  })
  .strict();
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const testResult = await testAdminAirbnbImportConnection(
      {
        propertyId: parsedPropertyId.data,
        candidateImportUrl: parsedRequest.data.candidateImportUrl,
      },
      actor,
    );
    const pageData = await getAdminExternalCalendarIntegrationsPage();
    return adminApiSuccessResponse({ pageData, testResult });
  } catch (error) {
