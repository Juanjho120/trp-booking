...
Status: **Functionally accepted; correction pending**
...
```text
Add explicit maxWait/timeout to both Serializable interactive transactions.
Retry complete idempotent transactions on P2034 up to three attempts.
Keep P2002/idempotency and opposite-decision conflict handling unchanged.
Anchor the datepicker popup directly to the trigger wrapper.
Do not change lifecycle states, pricing, holds, payments, schema, dependencies, env, visible copy, or the public 15-minute hold.
```

Correction document: docs/108-phase-11.5.3.1-transaction-resilience-and-datepicker-positioning.md.
