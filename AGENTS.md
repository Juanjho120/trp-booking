...
- Current package: Final-F — Public WhatsApp Contact and Admin Notifications — Active under accepted Final-F.R3 architecture rebaseline; Final-F.6 implementation is completed in repository with Local/Test migration applied, Hosted Test operational Web Push validation and explicit owner acceptance pending.
...
- Final-F.6 implementation record: `docs/203-final-f-6-admin-web-push-operational-notifications.md`.
- Final-F.6 — Admin Web Push operational notifications for `RESERVATION_CONFIRMED`, `RESERVATION_CANCELLED`, `CHECK_IN_MINUS_48H`, `CHECK_OUT_MINUS_6H`, and `REVIEW_SUBMITTED` — Implementation completed in repository with Local/Test migration applied; Hosted Test operational Web Push validation and explicit owner acceptance pending.
...
- `docs/203-final-f-6-admin-web-push-operational-notifications.md` is the Final-F.6 implementation record; Final-F.6 implementation is completed with Hosted Test operational Web Push validation and explicit owner acceptance pending.
...
- Final-F.R3 defines six target ADMIN operational notification classes. Final-F.6 implements the first five: `RESERVATION_CONFIRMED`, `RESERVATION_CANCELLED`, `CHECK_IN_MINUS_48H`, `CHECK_OUT_MINUS_6H`, and `REVIEW_SUBMITTED`. `GUEST_EMAIL_RECEIVED` remains reserved for Final-F.7 and must not be implemented before that subphase is explicitly requested.
